// Cloudflare Worker 优选IP可视化面板 - 完整版
// 版本: v2.9.5
...
async function saveDataSources(env, sources) {
  ...
  if (validSources.length === 0) {
    return { success: false, error: '至少需要一个有效的URL' };
  }
...
async function updateHighQualityPool(env, ip, latency) {
  ...
  try {
    ...
    if (currentCount > advancedConfig.maxHighQualityPoolSize) {
      await env.DB.prepare(
        `DELETE FROM high_quality_ips 
         WHERE ip IN (SELECT ip FROM high_quality_ips ORDER BY latency DESC LIMIT ?)`
      ).bind(currentCount - advancedConfig.maxHighQualityPoolSize).run();
    }
  } catch (e) {
...
async function updateIPs(env) {
  ...
  await addSystemLog(env, `🔄 IP列表已更新: ${ipList.length} 个IP (来自 ${sources.length} 个数据源)`);
  return ipList;
}
...
// ========== 获取高级配置 ==========
async function handleGetAdvancedConfig(env) {
  const config = await getAdvancedConfig(env);
...
// ========== 保存高级配置 ==========
async function handleSaveAdvancedConfig(request, env) {
  try {
    ...
    await addSystemLog(env, `⚙️ 高级配置已保存: 优质池容量=${config.maxHighQualityPoolSize}, 冷却天数=${config.failedIpCooldownDays}`);
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === '/settings') {
      return new Response(getSettingsHTML(env), {
        headers: { 'Content-Type': 'text/html;charset=UTF-8' }
      });
    }
    ...
    if (path === '/api/test-data-source' && request.method === 'POST') {
      ...
      if (!url) {
        return new Response(JSON.stringify({ success: false, error: 'URL不能为空' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' }
        });
      }
    ...
    if (path === '/api/get-advanced-config') {
      return handleGetAdvancedConfig(env);
    }

    if (path === '/api/save-advanced-config' && request.method === 'POST') {
      return handleSaveAdvancedConfig(request, env);
    }
...
<body>
  <div class="container">
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="form-group">
          <label>数据源列表</label>
          <div id="sourcesList" style="margin-bottom: 12px;">
            <!-- 动态加载数据源列表 -->
          </div>
      ...
      <div class="card-body">
        ...
        <div class="telegram-section" id="telegramConfig">
          ...
          <div class="checkbox">
            ...
            <div class="info-text" style="margin-left: 24px;">取消勾选将显示完整IP地址</div>
          </div>
        </div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        <p style="margin-bottom: 8px;">CF优选IP可视化面板</p>
  ...
  <script>
    ...
    async function resetDataSources() {
      if (!confirm('恢复默认数据源？当前自定义数据源将被清除')) {
        return;
      }
      ...
      try {
        ...
        if (data.success) {
          resultDiv.innerHTML = \`
            ✅ 测试成功<br>
            📊 状态码: \${data.status}<br>
            📝 CIDR数量: \${data.cidrCount}<br>
            🌐 IP数量: \${data.ipCount}<br>
            🔍 预览: \${data.preview.join(', ')}\${data.ipCount > 5 ? '...' : ''}
          \`;
