...
<img width="1350" height="597" alt="image" src="https://github.com/user-attachments/assets/33825235-292d-4cd0-a284-a1c823ad74b2" />
<img width="1349" height="594" alt="image" src="https://github.com/user-attachments/assets/ed304572-9089-444c-b426-42c18351c087" />
...
2. Cloudflare 账号
3. 一个域名（用于 DNS 更新）
4. Telegram Bot Token（可选，用于通知）

 创建 KV 命名空间
1. 进入存储和数据库选择 → Workers KV
2. 点击 创建命名空间
3. 名称填写：```CF_IP_KV```
4. 点击创建

 创建 D1 数据库
1. 进入 存储和数据库选择 → SQL数据库
2. 点击 创建数据库
3. 名称填写：```cf-ip-db```
4. 点击创建
...
3. 名称填写：```cf-best-ip```（或其他你喜欢的名字）
4. 点击 部署

配置 Worker
1. 点击 Worker 名称进入详情
2. 点击 编辑代码
3. 复制 ```混淆加密.js```(推荐）或者复制```woeker.js```
4. 点击 保存并部署

# 绑定 KV 命名空间
1. 在 Worker 详情页，点击 设置 → 变量
2. 找到 KV 命名空间绑定 部分
3. 点击 添加绑定
4. 变量名称：```KV```
5. KV 命名空间：选择你创建的 ```CF_IP_KV```
6. 点击 保存
...
|优质池数量|	实际测速数量	|说明|
|--------|---------|-----|
|5 个（严重不足）|	100个|	翻倍测速，快速补充|
|20 个（不足）|	50个	|使用用户设置值|
|40 个（充足）	|30个	|降低到30个，节省资源|
|60 个（已满）|	20个	|降低到20个，保鲜测速|

假设用户设置的测速数量为 100
|优质池数量|	实际测速数量	|说明|
|--------|---------|-----|
|5 个（严重不足）	|200个|	翻倍到上限200|
|20 个（不足）|	100个	|使用用户设置值|
|40 个（充足）|	30个	|降低到30个|
|60 个（已满）|	20个|	降低到20个|
...
|优质池数量|	实际测速数量	|说明|
|--------|---------|-----|
|5 个（严重不足）	|40个|	翻倍（20×2=40）|
|20 个（不足）|	20个	|使用用户设置值|
|40 个（充足）|	20个	|保持20个（未超过30上限）|
|60 个（已满）	|20个|	保持20个（未超过20上限）|

优先级顺序
1. 用户配置优先：测速数量以用户在界面设置的值为基础
2. 智能调整：根据优质池状态进行倍数调整或上限限制
3. 随机选择：从所有IP中随机选择（优先选择未测试过的IP）
4. 并发控制：使用用户设置的线程数控制并发

与手动测速的区别
|特性|	手动测速	|智能测速（定时任务）|
|--------|---------|-----|
|测速数量|	完全使用用户配置|	根据优质池状态动态调整|
|触发方式|	用户点击按钮|	定时任务自动触发|
|随机性	✅| 随机打乱	|✅ 随机打乱|
|优先级|	随机选择	|优先选择未测试IP|
|更新DNS	|可选	|可选（如果启用自动DNS）|

 🔧 运行参数
1. IP数量：只读显示，由环境变量 ```DEFAULT_IP_COUNT``` 控制。
2. 测速数量：自定义输入 (1-1000)，决定每次测速多少个 IP。
3. 线程数：自定义输入 (1-50)，决定测速的并发数。数值越大，测速越快。
4. 保存参数：保存“测速数量”和“线程数”的设置。

 📌 DNS 配置
1. API Token / Zone ID：输入你的 Cloudflare API 密钥和区域 ID。输入框会以密码形式隐藏已保存的值。
2. 域名：要更新的子域名（例如 cf.yourdomain.com）。
3. 代理状态：选择 DNS 记录的代理状态（橙色云/灰色云）。
4. 每小时自动更新DNS：勾选后，如果设置了 Cron 触发器，Worker 会每小时自动测速并更新 DNS。
5. 保存DNS / 立即更新：保存配置，或手动触发一次 DNS 更新。
