...
2. 删除默认代码
3. 选择以下**任一方式**上传代码：
   - **方式一**：复制 [`worker.js`](./worker.js) 全部内容粘贴进去（明文版本）
   - **方式二**：复制 [`混淆加密`](./混淆加密) 全部内容粘贴进去（混淆加密版本）
4. 点击 **保存并部署**
...
> **⚠️ 子请求限制说明**：
> Cloudflare Workers 免费版每个请求最多 50 个子请求。代码已优化，严格控制在 48 个子请求以内（留 2 个安全余量）：
>
> **单域名模式**（只设置主域名或只设置国家域名）：
> - **测速阶段**：最多 20 个 IP × 2 个子请求 = 40 个子请求
> - **DNS 更新阶段**：1（查询）+ 3（删除）+ 3（创建）+ 1（余量）= 8 个子请求
> - **通知**：1 个子请求
> - **总计**：40 + 8 + 1 = 49 个子请求 ✅
>
> **双域名模式**（同时设置主域名和国家域名）：
> - **测速阶段**：最多 15 个 IP × 2 个子请求 = 30 个子请求
> - **DNS 更新阶段**：8 × 2 个域名 = 16 个子请求
> - **通知**：1 个子请求
> - **总计**：30 + 16 + 1 = 47 个子请求 ✅
>
> **动态调整机制**：
